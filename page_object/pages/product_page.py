...
from page_object.pages.locators import ProductPageLocators
...
class ProductPage(BasePage):
    ...
    def should_be_success_adding_product_message(self):
        assert self.is_element_present(*ProductPageLocators.SUCCESS_ADDING_MESSAGE), "Message about adding product to basket is not displayed"
    ...
    def should_be_product_title_in_success_adding_product_message(self, product_title):
        ...
        assert "{} has been added to your basket.".format(product_title)==actual_message, "Incorrect success adding product message: {}". format(actual_message)

    def should_be_basket_price_message(self):
        assert self.is_element_present(*ProductPageLocators.BASKET_PRICE_MESSAGE), "Message about basket price is not displayed"

    def should_be_product_price_in_basket_message(self, product_price):
        actual_message = self.get_basket_price_message()
        assert "Your basket total is now {}".format(product_price)==actual_message, "Incorrect basket price message: {}". format(actual_message)